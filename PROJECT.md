...
### SQL-catcher TOs — detached, table-named

Decision (2026-07-13, Invoices cluster): keep one detached TO per base table, named identically to the table, to catch `ExecuteSQL`/epSQL `FROM "TableName"` queries. SQL resolves by TO name and ignores relationships, so detached is safe — and epSQL queries are preferentially based on these catchers. Current catcher set: `Invoices` (1065300), `InvoiceLines` (1065130), `InvoiceLinks` (1065126).

Hard rule: catcher TOs must NEVER be referenced in layout objects, hide/format conditions, or `TO::field` calc reads — those require a relationship and silently return empty. `Invoices_` (1065125) is NOT a catcher (underscore ≠ table name); it and `InvoiceLinks` are held for the final-phase anchor-buoy renaming, not for deletion.
...
| 2026-07-13 | Invoices cluster migration audited against fresh XML export — report in `agent/sandbox/PhaseB_Invoices_AuditReport.md`. ~85% landed; Stage 6 incomplete: legacy TO 1065125 renamed `Invoices_` (relationships stripped, not deleted) + undocumented new `Invoices` TO (1065300). 🔴 blockers before OttoFMS deploy: broken GTRR buttons on ServiceOrderDetails, dispatcher `print` verb missing + RačunŠtampa (183) not repointed, residual `Invoices_::` refs on layouts 171/204/239 and in live scripts 669/707/674/776. |
| 2026-07-15 | Invoices cluster audit findings all resolved. Dispatcher gained `generate` verb (delegates to 882, Commit before delegate was the fix for the runtime error; $gotoObject jump by fiscal state added); 673/650 retired. DP hotfix: `DP__AttachInvoices` (686) + `DP__ResolveInvoicesFromRange` (687) rewritten to epSQL against catcher TO `Invoices` (contracts unchanged, polog section kept native) — attach flow confirmed working live. Remaining: export-verification sweep, Init_Invoices disabled-step cleanup, Stage 7 OttoFMS deploy + live smoke, then the 1-month Retired/ clock (→ C.9). |
| 2026-07-15 | **I__ (Invoices) cluster migration marked DONE** (build + verification complete; punch list: two conditional-formatting formulas on 239, delete 489; Stage 7 deploy pending). Next cluster: **IncomingOrders (Primke) → ORD__**, migration begins Sunday 2026-07-19. Cluster scaffolded by developer (deliberately over-provisioned — prune pass needed). Caution: many layouts anchor on Primke-family tables; heavy business logic in this cluster. Survey + script audit done same day: keep 4 scaffold TOs, delete 6 (unused SOL-drill + returns branches); catcher convergence (octopus TOs already table-named — Stage 6 is relationship-strip, not delete); plans in `agent/sandbox/PhaseB_Primke_Inventory.md` + `PhaseB_Primke_Rollout.md`. |
| 2026-07-18 | **MPC-display bug on CalculationDetails (241) fixed** (deployed to live, soaking). Root cause: `MPCijena` (StavkeKalkulacijeMP calc) = `KMP__SKMP_StavkePrimke::ProdajnaCijena` passthrough — the source-of-truth selling price — but `KMP__SyncRUC`/`SyncPDV`/`SyncRabat` updated the internal amounts (RUCIznos/PDVIznos) without writing the recomputed price back to `StavkePrimke::ProdajnaCijena`, so editing RUC never moved MPC (or the printed PDF). Fix: write-back consolidated at end of `KMP__SyncPDV` (804) — `ProdajnaCijena = (prodajnaBezPDV + effPdvIznos)/Kolicina`; `SyncRabat` (802) now chains → SyncRUC → SyncPDV so Rabat propagates; all three gained an `$isRoot` **writability guard** (probe StavkePrimke via no-op epSQL UPDATE, retry 3×@1s, else `Revert Record/Request` + `Show Custom Dialog` "Zapis zaključan" + RECORD_LOCKED error — all-or-nothing). epSQL confirmed to respect FM record locks (per script 916). Sandbox: `KMP__Sync{PDV,RUC,Rabat}_-_ID_*.xml`. **Follow-up (not done):** `KMP__SyncFromMPCijena` (805) is dead wiring — its OnObjectSave sits on the uneditable `MPCijena` calc, so there is currently no working manual "type the final retail price" path. To restore it: bind that portal field to `StavkePrimke::ProdajnaCijena` and re-point the trigger at 805. |
