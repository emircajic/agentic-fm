...
| `reference-binding-on-paste.md` | FileMaker binds fmxmlsnippet object references by name, not by the `id` attribute: a valid-but-wrong ID is discarded and the ID rewritten from the name; `id="0"` and absent IDs resolve correctly. An unresolvable name is silently blanked to `id="0" name=""` with no error — and a blanked Set Field target falls back to the active field, writing wrong data silently. Name validation before paste is the only guard. Distinguishes paste-time binding from FM's internal ID-based storage | reference binding, paste, fmxmlsnippet, clipboard, field reference, script reference, layout reference, id attribute, name resolution, name wins, id ignored, advisory, silent blank, unresolvable, unknown field, hallucinated field, Set Field, active field, silent failure, fail open, R004, R006, R007, FMLint, CONTEXT.json, stale context, transport format |
